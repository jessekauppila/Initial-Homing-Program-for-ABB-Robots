...
!These are the functions to go from the 
    PROC nearLowToHome()
        MoveL NearSideLowAndPlate,Medium,z100,tool0\WObj:=wobj0;
        plateHomeToHome;
    ENDPROC
    ...
    !These Are the Functions To Go Across The top
    PROC farTopToNear()
    ...
    PROC moveHome()
        ...
        ! Calculates if the tool is near home
        !AND
        !Tool is Low
        !AND
        !Tool is NOT in homeposition...
